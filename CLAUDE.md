...
### Второй источник — конспекты со старого сайта

`python3 scripts/scrape_garden.py` выгружает раздел «Вопросы с собеседований по годам» со старого сайта (Quartz, interview-garden.vercel.app) в `inbox/garden/` — по markdown-файлу на собес, с сохранением вложенности списков. Скриншоты с условиями задач качаются отдельно в `inbox/garden/img/`. Перед вливанием страницы сверяются с уже разобранными видеозаписями: точные дубли того же собеса не добавляются (иначе завысится `asked`). Дальше — тот же путь: извлечение → `inbox/garden_questions/*.json` → дедупликация по бакетам.

В формулировках бывает многострочный код в ```…``` — на сайте он рендерится моноширинным блоком (`richText` в `main.ts`).
